...
public class DeliveryHateoasProcessor implements RepresentationModelProcessor<EntityModel<Delivery>>  {
    ...
    public EntityModel<Delivery> process(EntityModel<Delivery> model) {
        model.add(Link.of(model.getRequiredLink("self").getHref() + "/completedelivery").withRel("completedelivery"));
        model.add(Link.of(model.getRequiredLink("self").getHref() + "/startdelivery").withRel("startdelivery"));
