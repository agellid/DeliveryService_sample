<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-text>
            ...
            <String label="Foodstatus" v-model="value.foodstatus" :editMode="editMode"/>
            ...
            <Boolean label="IsPayed" v-model="value.isPayed" :editMode="editMode"/>
        </v-card-text>
    ...
    export default {
        ...
        methods: {
            ...
            async cookBegin() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['cookbegin'].href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            async cookEnd() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['cookend'].href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            async deliveryRequest() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['deliveryrequest'].href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
