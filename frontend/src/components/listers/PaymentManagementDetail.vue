<template>
    <v-card outlined>
        <v-card-title>
            PaymentManagement # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="PayStatus" v-model="item.payStatus" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/paymentManagements/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
