<template>
    <v-card outlined>
        <v-card-title>
            OrderData # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Boolean label="IsPayed" v-model="item.isPayed" :editMode="editMode" @change="change" />
            </div>
            <div>
                <Number label="AmountOfPayment" v-model="item.amountOfPayment" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/orderData/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
